...
```{r}
anno <- AnnotationDbi::select(org.Mm.eg.db,keys=results_ordered$ENSEMBL,
...
- Use `plotCounts` with the option `returnData=TRUE`  to get a data frame containing the counts of `ENSMUSG00000027750` in the different cell types. Visualise these data using `ggplot2`. 
...
One modification is to colour the points according to whether each gene is significant in the analysis. This is achieved by mapping the `col` aesthetic to a column in the table.
...
- Repeat the same heatmap as above, but for the top 100 most differentially-expressed genes **between lactation and virgin**
    + Load your `de_status.rds` file from last week. If you don't have one, [click here]()
...
```
- Modify the code producing the plot so that the points on the plot can be identified by `CellType` and `Status` (e.g using different colours and shapes or a facet). You will need to look at the options for `plotCounts`.
