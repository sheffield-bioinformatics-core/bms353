...
results_ordered <- arrange(results_cellType, padj)
```
...
```{r}
results_annotated <- left_join(results_ordered, anno,by="ENSEMBL")
...
- Use the option `returnData=TRUE` (as above) to get a data frame containing the counts of `ENSMUSG00000027750` in the different development stages. Visualise these data using `ggplot2`. 
...
- What does the `scale` argument in `pheatmap` do? Why might you want to scale by rows? Repeat one of your heatmaps, but scale the rows. What do you notice?
