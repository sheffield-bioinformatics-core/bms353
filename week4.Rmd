...
The function runs a couple of processing steps automatically to adjust for different library size and gene-wise variability, which you can read about in the [DESeq2 vignette](http://bioconductor.org/packages/devel/bioc/vignettes/DESeq2/inst/doc/DESeq2.html#the-deseq2-model).
...
resultsNames(de_status)
results_status <- data.frame(results(de_status))
```
...
- How many differentially expressed genes are found with adjusted p-value less than 0.05 and absolute log fold-change > 1 in the *multi-factor* analysis? How does this compare to the number of genes detected in the workshop exercise?
