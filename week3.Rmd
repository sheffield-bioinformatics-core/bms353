...
- What is the average number of transcripts-per-gene?
- Which gene has be most number of transcripts?
  + HINT: the `dplyr` function `count` might be useful here
</div>
...
```{r dgeLibrarySizes}
sampleinfo %>% mutate(LibSize=colSums(assay(dds))) %>% 
  ggplot(aes(x = Name, y = LibSize)) + geom_col()
