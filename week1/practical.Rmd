...
If you need to run analyses that require a lot of memoryyou will probably require access to a high-performance computing environment; such as sharc or bessemer at UoS. Analysis on such environments is performed via a terminal without a graphical interface. This means no RStudio, but you can still run commands via the R console. You can also write R scripts to perform your analysis.

## An R script

A new R script can be created via the menus in RStudio. The script is a file existing on your hard drive and can be used to document your analysis. In the screenshot below, an R script `myscript.R` was created to perform a few basic commands and make a plot from an in-built dataset in R.

![](images/script.PNG)

There is a limited ability to add explanations and commentary to the file via the use of comments; e.g. lines 3 and 5 in the screenshot that have a `#` at the start of the line.
...
- R markdown notebooks can facilitate reproducibile research
...
<div class="exercise">
Install the following set of packages
