...
The `rowMeans` function can be used to calculate the means of each row in a numeric matrix

### Exercise
<div class="exercise">
- Use the `rowMeans` function to calculate the average expression level for each gene
- Add a column to `test_results`
- Create the MA-plot

</div>

```{r}
mutate(test_results, means= rowMeans(normData)) %>% 
  ggplot(aes(x = means, y = dm)) + geom_point()
...
- Modify the volcano plot above so that points are coloured according to whether the gene is differentially-expressed or not with adjusted p-value < 0.05
  + HINT: you can use the `mutate` function to add an extra column to `test_results`
</div>
