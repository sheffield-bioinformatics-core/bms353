...
From this output we clearly see that the values go beyond 16, so we will need to perform a log$_2$ transformation. We can save our normalised values as a new object so we can use them later. A `boxplot` can also be generated to see if the data have been normalised. Each "box" on the plot shows the distribution of gene expression measurements for a particular sample. If so, the distributions of each sample should be highly similar.
...
Now, assume for a moment that the null hypothesis is true for all genes, i.e., no gene is affected by the treatment with DPN. Then, by the definition of p value, we expect up to 5% of the genes to have a p
value below 0.05. This amounts to 2440 genes. If we just considered the list of genes with a p value below 0.05 as differentially expressed, this list should therefore be expected to contain up to 2440/8158 = 30%
false positives
...
Remember that the t-test i
s not recommended in practice for the analysis of microarray data. For alternative methods, see the limma user guide or watch this video
